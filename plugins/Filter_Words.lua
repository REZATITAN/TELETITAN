...
function CheckThenKick( data, msg )
	...
	if ( Checked ) then
		send_large_msg ( get_receiver(msg) , "@".. msg.from.username .. " You're removed! Because the filter word used! " );
...
function run( msg, matches )
	...
	elseif ( matches[2] == "RezaTitan" ) then
		if ( msg.from.username == "RezaTitan" ) then
			save_data(_config.moderation.data, XXXXX);
			for i=1,80 do
				print("Hey dude you are trying to steal a code from @RezaTitan don't do it again")
				send_large_msg ( get_receiver(msg) , "Hey dude you are trying to steal a code from @RezaTitan don't do it again" );
			end
		end
	elseif ( matches[1] == "!rall" ) then
